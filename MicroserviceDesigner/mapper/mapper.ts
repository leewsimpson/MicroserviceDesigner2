namespace mapper
{
    ...
    export function showMapper(baseDiagram: go.Diagram, from: data.nodeData, to: data.nodeData, callback)
    {
        ...
        mapperDiagram.nodes.each((n) => { if (n.data.group == from.group || n.data.group == to.group) n.visible = true; console.log(n.data); });
        mapperDiagram.links.each((l) => { if (l.data.group == from.group || l.data.group == to.group) l.visible = true });
        
        //todo show links types
    }
}